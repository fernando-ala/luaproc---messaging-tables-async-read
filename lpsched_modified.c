    ...
    */
    ...
    /* has the lua process yielded? */
	  ...
	  /* yield due to assyncronous I/O */
		...
		//TODO: insert lock mechanism for queue aio
...
void *aioworkermain( void *args ) {
  luaproc *lp;
  //while true -> percorre a lista procurando AIO q acabaram
  while ( TRUE ) {    
  
	//TODO: fazer o locking na hora de retirar o LP da lista 
	pthread_mutex_lock( &mutex_aio_list );
...
void *testmain( void *args ) {
    //while (TRUE){
	puts("Eu sou a test main da WorkerTest!!!!");
	//}
}
/*end test*/
...
int sched_init( void ) {
  ...
  /*test*/
  pthread_t workerTest;
  pthread_create( &workerTest, NULL, testmain, NULL );
  /*end test*/
